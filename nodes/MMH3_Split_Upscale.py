...
修复: 每瓦片独立 Guider + 相位0 identity anchors + 独立 anchor noise-aug; probe 门控 polish
...
def warmup_keyframes(prev_video, prev_audio, sample_k0, fresh_k0, warmup_t):
    ...
    The generated warm-up is later discarded from the visible timeline.
    """
...
def boundary_identity_keyframes(source, conditioning_origin, token_indices, gate_strength):
    """Source-derived identity guards placed on exact global H3 token starts.

    These guards do not add temporal overlap.  They pin the last owned token of
    the previous chunk and the first owned token of the next chunk to the same
    source trajectory, using the ordinary identity-anchor strength and gate.
    """
    gate = max(0.0, min(1.0, float(gate_strength)))
    kfs = []
    seen = set()
    for k in token_indices:
        k = int(k)
        if k in seen or k < 0 or k >= source.shape[2]:
            continue
        seen.add(k)
        p = frames_for_tokens(k)
        kfs.append({
            "resolved_frame_index": int(p - conditioning_origin),
            "latent": source[:, :, k:k + 1].contiguous(),
            _MMH3_IDENTITY_GATE_KEY: gate,
            "_mmh3_boundary_identity_guard": True,
        })
    return kfs
...
class H3TemporalSplitParams(io.ComfyNode):
    ...
    def define_schema(cls):
        return io.Schema(
            ...
            description=("Temporal split controls. temporal_overlap_frames is regenerated by each neighboring chunk and linearly crossfaded in latent space during final assembly; total chunk window length stays fixed. anchor_strength is the single native H3 visual-condition strength used by identity anchors/guards and existing visual conditions."),
            inputs=[
                ...
                io.Float.Input("anchor_strength", default=0.999, min=0.0, max=1.0, step=0.001,
                               tooltip="Single stock H3 visual-condition noise-aug applied to identity anchors/guards and existing visual conditions in each split chunk."),
                ...
                io.Boolean.Input("boundary_identity_guards", default=True,
                                 tooltip="Add source-derived identity guards at each ownership cut using the same anchor_strength and identity gate."),
    ...
    def execute(cls, chunk_frames, temporal_overlap_frames, anchor_strength,
                ...
                boundary_identity_guards=True, absolute_timeline_positions=True) -> io.NodeOutput:
        ...
        return io.NodeOutput({"p": (chunk, overlap, float(anchor_strength),
                                    ...
                                    int(identity_anchor_frames), bool(boundary_identity_guards),
    ...
    def execute(cls, latent, conditioning, model, noise, sampler, sigmas,
        ...
        if temporal_split_param is not None:
            ...
            (cl, ov, anchor_strength, identity_anchor_gate, motion_n, identity_n,
             boundary_identity_guards, absolute_timeline_positions) = _tp[:8]
            ...
            print(f"[H3] boundary identity guards: {'ON' if boundary_identity_guards else 'OFF'}")
            if motion_n > 0:
                print("[H3] note: motion_anchor_frames is superseded by regenerated temporal overlap in this mode")
        else:
            bounds = [(0, 0, T, 0)]
            anchor_strength, identity_anchor_gate, motion_n, identity_n = 0.999, 1.0, 0, 0
            boundary_identity_guards = False
        ...
        for i, (sample_k0, fresh_k0, fresh_k1, warmup_t) in enumerate(bounds):
            ...
            # Auto source-identity conditions are built together so boundary guards
            # can de-duplicate a periodic anchor that already lands on the same token.
            periodic_ids = []
            if identity_n > 0:
            ...
            guard_ids = []
            if bool(boundary_identity_guards) and identity_n > 0:
                guard_tokens = []
                # First owned token of every non-first chunk: right side of cut.
                if i > 0 and fresh_k0 < fresh_k1:
                    guard_tokens.append(fresh_k0)
                # Last owned token of every non-final chunk: left side of cut.
                if i + 1 < len(bounds) and fresh_k1 > fresh_k0:
                    guard_tokens.append(fresh_k1 - 1)
                guard_ids = boundary_identity_keyframes(
                    source, sample_f0, guard_tokens, identity_anchor_gate)
                if guard_ids:
                    desc = ", ".join(
                        f"k={k} f={frames_for_tokens(k)}" for k in guard_tokens
                        if 0 <= int(k) < source.shape[2])
                    print(f"[H3] chunk {i}: boundary identity guard(s): {desc}")

            auto_ids = _merge_identity_keyframes(guard_ids, periodic_ids)
            if auto_ids:
